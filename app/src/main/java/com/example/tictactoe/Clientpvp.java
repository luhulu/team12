...
public class Clientpvp extends AppCompatActivity implements View.OnClickListener {
    ...
    private void judge() {
        ...
        if (A == 3 || B == 3 || C == 3 || D == 3 || E == 3 || F == 3 || G == 3 || H == 3) {
            tv_result.setText("A win");
            db.execSQL("INSERT INTO histore VALUES('"+"遠端連線" +"','"+"房主贏" +"');");
            pvp_result.com1_rlt++;
            prgstop();
        } else if (A == -3 || B == -3 || C == -3 || D == -3 || E == -3 || F == -3 || G == -3 || H == -3) {
            tv_result.setText("B win");
            db.execSQL("INSERT INTO histore VALUES('"+"遠端連線" +"','"+"你勝" +"');");
            pvp_result.com2_rlt++;
            prgstop();
        } else if ((a1 != 0 && a2 != 0 && a3 != 0 && a4 != 0 && a5 != 0 && a6 != 0 && a7 != 0 && a8 != 0 && a9 != 0)) {
            tv_result.setText("DRAW");
            db.execSQL("INSERT INTO histore VALUES('"+"遠端連線" +"','"+"平手" +"');");
            pvp_result.drw_rlt++;
            prgstop();
        }
    }
                    ...
                    try {
                        ...
                        try {
                            ...
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    Toast.makeText(getApplicationContext(),"遊戲有問題請重開",Toast.LENGTH_LONG).show();
                                }
                            });
                        }
                        finish();
